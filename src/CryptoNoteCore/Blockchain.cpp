...
// Copyright (c) 2016, The Karbowanec developers, The Brazukcoin developers
...
difficulty_type Blockchain::getDifficultyForNextBlock() {
  ...
  if (BlockMajorVersion >= BLOCK_MAJOR_VERSION_4) {
	 offset = m_blocks.size() - std::min(m_blocks.size(), static_cast<uint64_t>(m_currency.difficultyBlocksCount4()));
  ...
  return m_currency.nextDifficulty(BlockMajorVersion, timestamps, cummulative_difficulties);
}
...
difficulty_type Blockchain::get_next_difficulty_for_alternative_chain(const std::list<blocks_ext_by_hash::iterator>& alt_chain, BlockEntry& bei) {
	...
	if (BlockMajorVersion >= BLOCK_MAJOR_VERSION_2) {
		...
		if (alt_chain.size() < m_currency.difficultyBlocksCount2()) {
			...
			cummulative_difficulties.resize(std::min(alt_chain.size(), m_currency.difficultyBlocksCount2()));
			size_t count = 0;
